...
| OAuth scopes | INTEGRATE | authorization URL | Request `playlist-read-private`, `playlist-read-collaborative`, `playlist-modify-private`, plus Phase 12.6 playback scopes `streaming`, `user-read-email`, `user-read-private`. No history, library, or public-playlist scope. |
...
| Spotify playback / Web Playback SDK | INTEGRATE (Phase 12.6) | web Spotify detail | Premium Web Playback SDK on playlist detail; short-lived token via `GET /api/spotify/player/token`. Open in Spotify remains fallback. Harmonix-native songs still use Deezer 30s previews. |
| Spotify lyrics API | OPT-OUT | learning / detail | Spotify Web API does not expose lyrics. Detail **Lyrics** uses LRCLib with Spotify title/artist metadata. |
