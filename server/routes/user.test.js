...
describe('User Preferences API Routes', () => {
  ...
  describe('PATCH /preferences', () => {
    ...
    it('purges word queue when native_language changes', () => {
      const userId = 'up-test';
      db.prepare("UPDATE users SET native_language = 'en', target_language = 'es' WHERE id = ?").run(userId);
      db.prepare('DELETE FROM user_word_queue WHERE user_id = ?').run(userId);
      wordQueue.enqueuePayloads(userId, [{
        date: '2026-07-09',
        language_code: 'es',
      ...
      expect(wordQueue.countReady(userId)).to.equal(1);
      const handler = userRouter.stack.find(s => s.route.path === '/preferences' && s.route.methods.patch).route.stack[0].handle;
      const req = { body: { native_language: 'fr' }, user: { id: userId } };
      const res = mockRes();
      handler(req, res);
      expect(res.body.native_language).to.equal('fr');
      expect(wordQueue.countReady(userId)).to.equal(0);
    });
