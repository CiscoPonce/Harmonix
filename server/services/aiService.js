...
const GENRE_HIT_EXAMPLES = {
  ...
  en: {
    ...
    'hip-hop': 'Lose Yourself (Eminem), Not Afraid (Eminem), HUMBLE. (Kendrick Lamar), God\'s Plan (Drake), Stronger (Kanye West), In Da Club (50 Cent), Empire State of Mind (Jay-Z), The Real Slim Shady (Eminem), Without Me (Eminem), Till I Collapse (Eminem), Hotline Bling (Drake), One Dance (Drake), See You Again (Wiz Khalifa), Alright (Kendrick Lamar), Old Town Road (Lil Nas X), Rockstar (Post Malone), Congratulations (Post Malone)',
    any: 'Bad Guy (Billie Eilish), Rolling in the Deep (Adele), Heat Waves (Glass Animals), Mr. Brightside (The Killers)',
  },
  fr: {
    pop: 'Formidable (Stromae), On écrit sur les murs (Kids United), Tourner dans le vide (Indila), Je veux (Zaz), Avant nous (Soprano), Dernière Danse (Indila), Papaoutai (Stromae), Alors on danse (Stromae), Tous les mêmes (Stromae), Balance ton quoi (Angèle), Désenchantée (Mylène Farmer), Elle me dit (MIKA), Je vole (Louane), La vie en rose (Édith Piaf), Joe le taxi (Vanessa Paradis)',
    rock: 'Mistral gagnant (Renaud), Comme des enfants (Cœur de pirate), Homme Contant (Noir Désir), Le vent nous portera (Noir Désir), Dis-moi (BB Brunes), L\'aventurier (Indochine), J\'ai demandé à la lune (Indochine), Un autre monde (Téléphone), Je dis aime (-M-), J\'t\'emmène au vent (Louise Attaque), Trois nuits par semaine (Indochine), Cendrillon (Téléphone)',
    'hip-hop': 'Ailleurs (Orelsan), Basique (Orelsan), Tout va bien (Orelsan), Feu (Nekfeu), J\'suis pas bon (Nekfeu), La terre est ronde (Orelsan), On verra (Nekfeu), Mauvaise graine (Nekfeu), Je danse le mia (IAM), Nés sous la même étoile (IAM), Bouge de là (MC Solaar), Caroline (MC Solaar)',
    any: 'Formidable (Stromae), Je veux (Zaz), Mistral gagnant (Renaud), Tourner dans le vide (Indila)',
  },
  de: {
    pop: 'Atemlos durch die Nacht (Helene Fischer), Männer (Herbert Grönemeyer), 99 Luftballons (Nena), Auf uns (Andreas Bourani), Das Beste (Silbermond), Perfekte Welle (Juli), Leuchtturm (Nena), Irgendwie irgendwo irgendwann (Nena), Ein Kompliment (Sportfreunde Stiller), Applaus Applaus (Sportfreunde Stiller), Stark (Ich + Ich), Vom selben Stern (Ich + Ich), Geboren um zu leben (Unheilig)',
    rock: 'Du hast (Rammstein), Durch den Monsun (Tokio Hotel), Engel (Rammstein), Tage wie diese (Die Toten Hosen), Nur ein Wort (Wir sind Helden), Sonne (Rammstein), Ich will (Rammstein), Mein Herz brennt (Rammstein), Schrei (Tokio Hotel), Übers Ende der Welt (Tokio Hotel), Bonnie und Clyde (Die Toten Hosen), Westerland (Die Ärzte), Männer sind Schweine (Die Ärzte), Schrei nach Liebe (Die Ärzte)',
    'hip-hop': 'Willst Du (Alligatoah), Fremdgehen (Alligatoah), Du bist schön (Alligatoah), Traurig (Apache 207), Roller (Apache 207), Haus am See (Peter Fox), Alles neu (Peter Fox), Schwarz zu blau (Peter Fox), Easy (Cro), Bye Bye (Cro), Einmal um die Welt (Cro), 200 km/h (Apache 207)',
  ...
  pt: {
    pop: 'Ai Se Eu Te Pego (Michel Teló), Envolver (Anitta), Garota de Ipanema (Tom Jobim), Balada (Gusttavo Lima), Olha a Explosão (MC Kevinho), Evidências (Chitãozinho & Xororó), Show das Poderosas (Anitta), Infiel (Marília Mendonça), Chega de Saudade (João Gilberto), Mas Que Nada (Sérgio Mendes), Aquarela (Toquinho), Exagerado (Cazuza), Malandragem (Cássia Eller), Segundo Sol (Cássia Eller)',
    rock: 'Eduardo e Mônica (Legião Urbana), Tempo Perdido (Legião Urbana), Pais e Filhos (Legião Urbana), Anna Júlia (Los Hermanos), É Preciso Saber Viver (Titãs), Faroeste Caboclo (Legião Urbana), Índios (Legião Urbana), Que País É Este (Legião Urbana), Epitáfio (Titãs), Flores (Titãs), Meu Erro (Os Paralamas), Lanterna dos Afogados (Os Paralamas)',
    'hip-hop': 'Rap é Compromisso (Racionais MC\'s), Diário de um Detento (Racionais MC\'s), Negro Drama (Racionais MC\'s), Capítulo 4 Versículo 3 (Racionais MC\'s), Vida Loka (Racionais MC\'s), A Vida É Desafio (Racionais MC\'s), Boa Esperança (Emicida), AmarElo (Emicida), Não Existe Amor em SP (Criolo), Tombei (Karol Conká), Passarinhos (Emicida), Subirusdoistiozin (Criolo)',
  ...
  it: {
    pop: 'Più bella cosa (Eros Ramazzotti), Sere nere (Tiziano Ferro), Guerriero (Marco Mengoni), Con te partirò (Andrea Bocelli), Sarà perché ti amo (Ricchi e Poveri), Gloria (Umberto Tozzi), Laura non c\'è (Nek), L\'essenziale (Marco Mengoni), A te (Jovanotti), Perdono (Tiziano Ferro), La solitudine (Laura Pausini), Almeno tu nell\'universo (Mia Martini), Se bastasse una canzone (Eros Ramazzotti)',
    rock: 'Zitti e buoni (Måneskin), Certe Notti (Ligabue), Bello e impossibile (Gianna Nannini), Urlando contro il cielo (Ligabue), Meraviglioso (Negramaro), Torna a casa (Måneskin), Albachiara (Vasco Rossi), Vivere (Vasco Rossi), Siamo solo noi (Vasco Rossi), Solo 3 minuti (Negramaro), Mentre tutto scorre (Negramaro), Una vita da mediano (Ligabue)',
    'hip-hop': 'Starboy (Sfera Ebbasta), Pablo (Sfera Ebbasta), Madonna (Sfera Ebbasta), Cosa nostra (Marracash), Brutti e cattivi (Guè), Cara Italia (Ghali), Soldi (Mahmood), In Italia (Fabri Fibra), Tranne te (Fabri Fibra), Vengo dalla Luna (Caparezza), Eroe (Caparezza), Rockstar (Sfera Ebbasta)',
